...
(1, 'vinay@gmail.com', 'vinay', 'Cardiologists'),
(2, 'nayana@gmail.com', 'nayana', 'Dermatologists'),
(3, 'mahi@gmail.com', 'mahi', 'Anesthesiologists'),
(4, 'joel@gmail', 'joel', 'Endocrinologists'),
(5, 'meghana@gmail.com', 'meghana', 'corona');
...
--
DELIMITER $$
CREATE PHARMACY `PatientDelete` BEFORE DELETE ON `patients` FOR EACH ROW INSERT INTO pharmacy VALUES(null,OLD.pid,OLD.email,OLD.name,'PATIENT DELETED',NOW())
$$
DELIMITER ;
DELIMITER $$
CREATE PHARMACY `PatientUpdate` AFTER UPDATE ON `patients` FOR EACH ROW INSERT INTO pharmacy VALUES(null,NEW.pid,NEW.email,NEW.name,'PATIENT UPDATED',NOW())
$$
DELIMITER ;
DELIMITER $$
CREATE PHARMACY `patientinsertion` AFTER INSERT ON `patients` FOR EACH ROW INSERT INTO pharmacy VALUES(null,NEW.pid,NEW.email,NEW.name,'PATIENT INSERTED',NOW())
$$
DELIMITER ;
...
INSERT INTO `pharmacy` (`PHARM_id`, `pid`, `email`, `name`, `action`, `timestamp`) VALUES
...
--
ALTER TABLE `trigr`
  MODIFY `PHARM_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=20;
